...
const ORBITS: { key: TabKey; label: string; radius: number; color: string; colorBg: string; iconColor: string; labelAngle: number }[] = [
  { key: "core", label: "SAF+ Core", radius: 130, color: "hsl(352, 88%, 43%)", colorBg: "bg-primary/10 text-primary border-primary/20", iconColor: "text-primary", labelAngle: -90 },
  { key: "integraciones", label: "Integraciones", radius: 250, color: "hsl(38, 92%, 50%)", colorBg: "bg-amber-50 text-amber-600 border-amber-200", iconColor: "text-amber-500", labelAngle: -90 },
  { key: "operaciones", label: "Operaciones", radius: 350, color: "hsl(199, 89%, 48%)", colorBg: "bg-sky-50 text-sky-600 border-sky-200", iconColor: "text-sky-500", labelAngle: -90 },
];
...
function OrbitRings({ cx, cy, activeOrbit, isInView }: { cx: number; cy: number; activeOrbit: TabKey | null; isInView: boolean }) {
  return (
    <>
      {ORBITS.map((orbit, oi) => (
        <circle
          ...
          style={{ opacity: isInView ? 1 : 0, transitionDelay: `${oi * 200}ms` }}
        />
      ))}
    </>
  );
...
function OrbitLabels({ cx, cy, activeOrbit, setActiveOrbit, setActiveModule, isInView }: {
...
function ModuleNodes({ cx, cy, activeOrbit, activeModule, setActiveOrbit, setActiveModule, isInView }: {
  cx: number; cy: number; activeOrbit: TabKey | null;
  activeModule: { name: string; desc: string } | null;
  setActiveOrbit: (v: TabKey) => void; setActiveModule: (v: { name: string; desc: string }) => void; isInView: boolean;
}) {
...
function FilterButtons({ activeOrbit, setActiveOrbit, setActiveModule }: {
  activeOrbit: TabKey | null; setActiveOrbit: (v: TabKey | null) => void; setActiveModule: (v: null) => void;
}) {
  const showAll = activeOrbit === null;
  return (
...
function DetailPanel({ activeOrbit, activeModule, setActiveOrbit, setActiveModule }: {
  activeOrbit: TabKey | null; activeModule: { name: string; desc: string } | null;
  setActiveOrbit: (v: TabKey | null) => void; setActiveModule: (v: { name: string; desc: string } | null) => void;
}) {
  const showAll = activeOrbit === null;
  ...
  if (activeModule) {
    return (
      <div className="bg-card rounded-2xl border p-8 shadow-sm animate-fade-in">
        ...
        <button onClick={() => setActiveModule(null)} className="mt-4 text-xs text-primary font-medium hover:underline active:scale-95 transition-transform">
    ...
    <section ref={sectionRef} id="modulos" className="py-24 px-6 bg-secondary/50">
      <div className="max-w-7xl mx-auto">
        <div className="scroll-reveal mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-foreground text-balance leading-tight">Módulos SAF+</h2>
          <p className="text-muted-foreground mt-2 text-sm max-w-xl">Explora el ecosistema SAF+ — selecciona una capa para ver sus módulos.</p>
        </div>
        ...
        <div className="scroll-reveal flex flex-col xl:flex-row gap-10 items-start">
          ...
          <div className="relative shrink-0 hidden lg:block" style={{ width: size, height: size }}>
            <svg width={size} height={size} className="absolute inset-0 z-0" aria-hidden>
            ...
            <div
              ...
              style={{ width: 90, height: 90, left: cx - 45, top: cy - 45, opacity: isInView ? 1 : 0, transition: "opacity 0.6s, transform 0.5s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.5s" }}
              onClick={() => { setActiveOrbit(null); setActiveModule(null); }}
            >
              <span className="text-white font-bold text-sm text-center leading-tight">SAF+</span>
            </div>

            <ModuleNodes cx={cx} cy={cy} activeOrbit={activeOrbit} activeModule={activeModule} setActiveOrbit={setActiveOrbit} setActiveModule={setActiveModule} isInView={isInView} />
            <OrbitLabels cx={cx} cy={cy} activeOrbit={activeOrbit} setActiveOrbit={setActiveOrbit} setActiveModule={setActiveModule} isInView={isInView} />
          </div>

          {/* Detail panel */}
          <div className="flex-1 min-w-0 w-full">
            <FilterButtons activeOrbit={activeOrbit} setActiveOrbit={setActiveOrbit} setActiveModule={() => setActiveModule(null)} />
            <DetailPanel activeOrbit={activeOrbit} activeModule={activeModule} setActiveOrbit={setActiveOrbit} setActiveModule={setActiveModule} />
          </div>
        </div>
      </div>
    </section>
